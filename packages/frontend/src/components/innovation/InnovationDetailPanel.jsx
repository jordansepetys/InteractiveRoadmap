  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[480px] bg-white shadow-xl z-50 flex flex-col">
          ...
          <div className="flex items-center gap-2">
            {!isEditing ? (
              <button
                ...
                className="px-3 py-1.5 text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded"
              >
                Edit
              </button>
            ) : (
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-sm text-slate-600">{item.requestor || '-'}</p>
              )}
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-50 rounded-lg">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-slate-700">RICE Score</h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Reach (users/period)</label>
                {isEditing ? (
                  <input
                    type="number"
                  ...
                  <p className="text-sm font-medium text-slate-700">{item.rice_reach ?? '-'}</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Impact (1-3)</label>
                {isEditing ? (
                  <select
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm font-medium text-slate-700">{item.rice_confidence ? `${item.rice_confidence}%` : '-'}</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Effort (person-weeks)</label>
                {isEditing ? (
                  <input
                    type="number"
                  ...
                  <p className="text-sm font-medium text-slate-700">{item.rice_effort ?? '-'}</p>
                )}
              </div>
            </div>
          </div>
          ...
          {/* ROI */}
          <div>
            <h3 className="text-sm font-semibold text-slate-700 mb-3">ROI Estimate</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Estimate</label>
                {isEditing ? (
                  <input
                    type="text"
                  ...
                  <p className="text-sm text-slate-600">{item.roi_estimate || '-'}</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">ROI Notes / Justification</label>
                {isEditing ? (
                  <textarea
                  ...
                  <p className="text-sm text-slate-600 whitespace-pre-wrap">{item.roi_notes || '-'}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {isEditing ? (
              ...
              <p className="text-sm text-slate-600 whitespace-pre-wrap">{item.status_notes || 'No status notes'}</p>
            )}
          </div>
          ...
          {item.stage === 'Rejected' && (
            <div className="p-4 bg-red-50 rounded-lg border border-red-200">
              ...
              {isEditing ? (
                ...
                <p className="text-sm text-red-700">{item.rejection_reason || 'No reason provided'}</p>
              )}
            </div>
          )}
          ...
          {item.ado_feature_id && (
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-sm text-blue-700">Feature #{item.ado_feature_id}</p>
            </div>
          )}
        ...
        {/* Footer */}
        <div className="flex-none px-6 py-4 border-t border-slate-200">
          <button
